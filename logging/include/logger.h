...
class Logger : public Singleton<Logger> {
  public:
    void Log(const std::string &level, const std::string &component, const std::string &file, const std::string &func,
...
#define LOG(level, component, msg)                                                                                     \
    Logger::Get().Log(level, component, __FILE__, __func__,                                                            \
                      (static_cast<std::stringstream &>(std::stringstream().flush() << msg)).str());
